...
**The custom form builder can be enabled/disabled by adding `\VanOns\FilamentFormBuilder\View\Components\Forms\CustomForm::class => <label>` to the `templates` array in the `filament-form-builder.php` config file.**
...
**Tip: look in the existing fields for examples.**
...
This adds the `recaptcha` rules to the form, if you don't do this, the captcha will not be validated.
